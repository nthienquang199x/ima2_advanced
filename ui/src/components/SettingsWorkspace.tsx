        ...
        <div className="settings-layout">
          ...
          <section ref={contentRef} className="settings-content" aria-label={t("settings.contentAria")}>
            <SettingsSectionBlock id="providers" setRef={setSectionRef}>
              ...
              ) : provider === "agy" || provider === "gemini-api" || provider === "gemini-web" ? (
                <article className="settings-row">
                  <div className="settings-row__copy">
                    <h4>{provider === "gemini-api" ? t("provider.geminiApiCompatTitle") : provider === "gemini-web" ? t("provider.geminiWebCompatTitle") : t("provider.agyCompatTitle")}</h4>
                    <p>{provider === "gemini-api" ? t("provider.geminiApiCompatBodyLong") : provider === "gemini-web" ? t("provider.geminiWebCompatBodyLong") : t("provider.agyCompatBodyLong")}</p>
                  </div>
                </article>
              ) : (
                <>
