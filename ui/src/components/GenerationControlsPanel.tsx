...
export function GenerationControlsPanel() {
  ...
  const isAnyGemini = isAgyOnly || isGeminiApi || isGeminiWeb;
  ...
  const providerCompat = isGrok
    ...
    : isAgyOnly
      ...
      : isGeminiApi
        ...
        : isGeminiWeb
          ? { title: t("provider.geminiWebCompatTitle"), body: t("provider.geminiWebCompatBody") }
          : isNai
  ...
  return (
    <div className="right-panel-settings" role="tabpanel">
      ...
      {isAgyOnly ? null : isGrok ? (
        ...
        // The bridge (gemini-web-bridge/server.py) takes no aspect-ratio/size
        // params — the web app has none — so only the model choice renders,
        // unlike gemini-api's full aspect/resolution grid below.
