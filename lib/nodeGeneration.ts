...
export async function runNodeGeneration(req: Request, res: Response, ctx: RuntimeContext) {
    ...
    try {
      ...
      if ((activeProvider === "grok" || activeProvider === "agy" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web") && inputImageCount > providerReferenceLimit!) {
        finishStatus = "error";
        finishHttpStatus = 400;
      ...
      let resultFormat: "png" | "jpeg" | "webp" = activeProvider === "grok" || activeProvider === "agy" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web" || activeProvider === "atlascloud" || activeProvider === "minimax" ? "jpeg" : format as "png" | "jpeg" | "webp";
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
          ...
          const r = activeProvider === "gemini-api"
            ...
            ? await generateViaGeminiWeb(parentB64 ? `Edit this image: ${generationPrompt}` : generationPrompt, requireRuntimeContext(ctx), {
                model: effectiveImageModel,
                signal: cancelController.signal,
                requestId,
                references: parentB64
          ...
          if (r.b64) {
            ...
            if (activeProvider === "grok" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web" || activeProvider === "atlascloud" || activeProvider === "minimax" || activeProvider === "nai") {
