...
async function resolveMultimodeElements(
  ...
  try {
    ...
    const provider = activeProvider === "grok" || activeProvider === "grok-api" ? "grok"
      : (activeProvider === "gemini-api" || activeProvider === "gemini-web") ? "gemini" : "gpt";
...
export async function runMultimodePipeline(req: Request, res: Response, ctx: RuntimeContext) {
    ...
    try {
     ...
     const elementResult = rawElementIds.length > 0
      ...
      const mmFormat = activeProvider === "grok" || activeProvider === "agy" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web" || activeProvider === "atlascloud" || activeProvider === "minimax" ? "jpeg" : String(format);
      ...
      const persistAndSendImage = async ( image: MultimodeImage, index: number, totalReturned: number, status: ReturnType<typeof sequenceStatus>, ) => {
        ...
        const resultMime = activeProvider === "grok" || activeProvider === "agy" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web" || activeProvider === "atlascloud" || activeProvider === "minimax" || activeProvider === "nai"
          ? (image.mime || detectImageMimeFromB64(image.b64) || mime)
          : mime;
        const resultFormat = activeProvider === "grok" || activeProvider === "agy" || activeProvider === "grok-api" || activeProvider === "gemini-api" || activeProvider === "gemini-web" || activeProvider === "atlascloud" || activeProvider === "minimax" || activeProvider === "nai" ? imageFormatFromMime(resultMime) : mmFormat;
